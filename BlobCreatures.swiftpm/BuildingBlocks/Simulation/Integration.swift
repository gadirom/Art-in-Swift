...
struct Integration<T: SimulatableParticle>: MetalBuildingBlock {
    ...
    static func addUniforms(_ desc: inout UniformsDescriptor){
        desc = desc
            ...
            .float("iFric", range: 0...2, value: 0.99)
    }
    ...
    var metalContent: MetalContent{
        Compute("integration")
             ...
             kernel void integration(uint id [[thread_position_in_grid]]){
                 ...
                 p.prevCoord = p.coord;
                 velo += -u.grav*0.01*normalize(p.coord);//   gravity*0.01;
                 ...
                 p.coord = mix(p.coord, p.prevCoord, u.iFric);
                 ...
                 if (p.coord.y<-canvas.y/2){p.prevCoord.y=p.coord.y; p.coord.y=-canvas.x/2;}
             
                 if(p.connected) p.color = max(0.,  p.color-u.lSub);
